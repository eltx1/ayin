...
function replaceOnce(relative, search, replacement) {
  const target = file(relative);
  const source = fs.readFileSync(target, "utf8");
  if (source.includes(replacement)) return;
  const index = source.indexOf(search);
  if (index < 0) throw new Error(`Patch anchor not found in ${relative}`);
  const next = source.slice(0, index) + replacement + source.slice(index + search.length);
...
export class MediaProcessingQueueService {
  ...
  async claimNext(workerId: string) {
    ...
    return this.database.client.$transaction(async (tx) => {
      await tx.$queryRawUnsafe(`SELECT pg_advisory_xact_lock(${QUEUE_ADVISORY_LOCK})`);
...
databaseDescribe("media processing queue global concurrency", () => {
  ...
  async function register(suffix: string) {
    const auth = moduleReference.get("AuthService", { strict: false }) as {
  ...
  async function createQueuedJob(suffix: string, status: "QUEUED" | "READY" | "PROCESSING" = "QUEUED") {
    const identity = await register(suffix);
    ...
    return prisma.mediaProcessingJob.create({
      data: {
        ...
        outputR2ObjectKey: `channels/${identity.channelId}/media/${video.id}/processed-v1.mp4`,
  ...
  it("enforces a database-global concurrency cap and expands immediately when admin raises it", async () => {
    ...
    const claimOne = await queue.claimNext("worker-a");
    expect(claimOne?.id).toBe(first.id);
    expect(await queue.claimNext("worker-b")).toBeNull();

    await setSetting("mediaProcessingConcurrentJobs", 2);
    const claimTwo = await queue.claimNext("worker-b");
  ...
  it("pauses new claims dynamically while leaving queued work intact", async () => {
    ...
    expect(await queue.claimNext("worker-a")).toBeNull();
    const stored = await prisma.mediaProcessingJob.findUnique({ where: { id: queued.id } });
...
console.log("Media processing queue phase 1 patch applied.");
