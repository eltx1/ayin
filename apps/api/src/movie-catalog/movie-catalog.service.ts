...
export interface MovieCatalogPatch extends Partial<Omit<MovieCatalogInput, "genres" | "artwork" | "availability" | "localizations">> {
  genres?: string[];
  artwork?: MovieArtworkInput[];
  availability?: MovieAvailabilityInput[];
  localizations?: MovieLocalizationInput[];
...
export class MovieCatalogService {
  ...
  async create(input: MovieCatalogInput) {
    ...
    await this.assertReferencedMedia(normalized.primaryVideoId, normalized.trailerVideoId, normalized.artwork);
    ...
    try {
      const movie = await this.database.client.$transaction(async (tx) => {
        ...
        for (const name of normalized.genres) {
          ...
          if (!slug) throw movieError(400, "INVALID_GENRE", "Every genre must contain a safe name.");
          genres.push(
      ...
      if (isUniqueConstraint(error)) throw movieError(409, "MOVIE_SLUG_CONFLICT", "That movie slug is already in use.");
      throw error;
    }
  }
  ...
  async update(movieId: string, patch: MovieCatalogPatch) {
    ...
    if (current.status === "ARCHIVED") throw movieError(409, "MOVIE_ARCHIVED", "Archived movies cannot be edited.");

    const scalar = this.normalizePatch(current, patch);
    const artwork = patch.artwork ? this.normalizeArtwork(patch.artwork) : undefined;
    const availability = patch.availability ? this.normalizeAvailability(patch.availability) : undefined;
    const localizations = patch.localizations ? this.normalizeLocalizations(patch.localizations) : undefined;
    await this.assertReferencedMedia(
      scalar.primaryVideoId ?? current.primaryVideoId,
      scalar.trailerVideoId ?? current.trailerVideoId,
      artwork ?? current.artwork.map((item) => ({ type: item.type, mediaAssetId: item.mediaAssetId, altText: item.altText })),
    );
      ...
      await this.database.client.$transaction(async (tx) => {
        ...
        if (patch.genres) {
          ...
          for (const nameRaw of uniqueTrimmed(patch.genres)) {
            ...
            await tx.movieGenreAssignment.create({ data: { movieId, genreId: genre.id, position } });
            position += 1;
          }
        }
        if (artwork) {
          await tx.movieArtwork.deleteMany({ where: { movieId } });
          if (artwork.length) await tx.movieArtwork.createMany({ data: artwork.map((item) => ({ movieId, ...item })) });
        }
        if (availability) {
          await tx.movieAvailability.deleteMany({ where: { movieId } });
          if (availability.length) await tx.movieAvailability.createMany({ data: availability.map((item) => ({ movieId, ...item })) });
        }
        if (localizations) {
          await tx.movieLocalization.deleteMany({ where: { movieId } });
          if (localizations.length) await tx.movieLocalization.createMany({ data: localizations.map((item) => ({ movieId, ...item })) });
        }
      });
  ...
  async publish(movieId: string) {
    ...
    const issues = moviePublishIssues({
      ...movie,
      artwork: movie.artwork.map((item) => ({ type: item.type, assetReady: item.asset?.status === "VALIDATED" && !item.asset.removedAt })),
    });
    if (issues.length) {
      throw movieError(409, "MOVIE_NOT_PUBLISHABLE", "Movie is missing required publish data.", { issues });
    }
  ...
  async unpublish(movieId: string) {
    ...
    await this.database.client.movie.update({ where: { id: movieId }, data: { status: "DRAFT", publishedAt: null } });
    return this.getAdminById(movieId);
  }

  async archive(movieId: string) {
    await this.requireMovie(movieId);
    await this.database.client.movie.update({ where: { id: movieId }, data: { status: "ARCHIVED", publishedAt: null } });
    return this.getAdminById(movieId);
  }
  ...
  async listAdmin(limit = 50) {
    const rows = await this.database.client.movie.findMany({
      ...
      include: { genres: { include: { genre: true } }, artwork: true, availability: true, localizations: true },
    });
  ...
  async getPublicBySlug(slugRaw: string, countryCode?: string | null) {
    ...
    const videoDecision = await this.videoPolicy.decide(row.primaryVideoId, { countryCode: countryCode ?? undefined });
  ...
  async listPublic(countryCode?: string | null, limit = 24) {
    ...
    const ids = rightsVisible.map((movie) => movie.primaryVideoId).filter((value): value is string => Boolean(value));
    const videoDecisions = await this.videoPolicy.decideMany(ids, { countryCode: countryCode ?? undefined });
  ...
  private async hydrate<T extends Awaited<ReturnType<MovieCatalogService["getAdminRow"]>>>(row: NonNullable<T>) {
    const videoIds = [row.primaryVideoId, row.trailerVideoId].filter((value): value is string => Boolean(value));
    const assetIds = row.artwork.map((item) => item.mediaAssetId);
    const [videos, assets] = await Promise.all([
      videoIds.length ? this.database.client.video.findMany({ where: { id: { in: videoIds } }, select: { id: true, slug: true, title: true, status: true, visibility: true, durationMs: true } }) : [],
      assetIds.length ? this.database.client.mediaAsset.findMany({ where: { id: { in: assetIds } }, select: { id: true, r2ObjectKey: true, mimeType: true, status: true, width: true, height: true, removedAt: true } }) : [],
    ]);
    ...
    return {
      ...
      artwork: row.artwork.map((item) => ({ ...item, asset: assetsById.get(item.mediaAssetId) ?? null })),
    };
  }

  private isPubliclyAvailable(movie: Awaited<ReturnType<MovieCatalogService["getAdminById"]>>, countryCode?: string | null) {
    return Boolean(
  ...
  private normalizeInput(input: MovieCatalogInput) {
    ...
    if (!isSafeMovieSlug(slug)) throw movieError(400, "INVALID_MOVIE_SLUG", "A safe ASCII movie slug is required.");
    return {
  ...
  private normalizePatch(current: { title: string; slug: string }, patch: MovieCatalogPatch) {
    ...
    if (patch.synopsis !== undefined) data.synopsis = requiredText(patch.synopsis, "synopsis", 20_000);
    if (patch.releaseDate !== undefined) data.releaseDate = patch.releaseDate;
    if (patch.releaseYear !== undefined) data.releaseYear = checkedInteger(patch.releaseYear, "releaseYear", 1888, 2200);
    if (patch.runtimeMinutes !== undefined) data.runtimeMinutes = checkedInteger(patch.runtimeMinutes, "runtimeMinutes", 1, 1440);
    if (patch.maturityRating !== undefined) data.maturityRating = requiredText(patch.maturityRating, "maturityRating", 32);
    if (patch.originalLanguage !== undefined) data.originalLanguage = requiredText(patch.originalLanguage, "originalLanguage", 16).toLowerCase();
  ...
  private normalizeArtwork(items: MovieArtworkInput[]) {
    ...
    return items.map((item) => {
      if (seen.has(item.type)) throw movieError(400, "DUPLICATE_ARTWORK_TYPE", `Artwork type ${item.type} is duplicated.`);
      seen.add(item.type);
      return { type: item.type, mediaAssetId: item.mediaAssetId, altText: item.altText?.trim() || null };
    });
  }
  ...
  private normalizeAvailability(items: MovieAvailabilityInput[]) {
    return items.map((item) => {
      ...
      if (!territoryCode) throw movieError(400, "INVALID_TERRITORY", "Territory must be a two-letter country code or '*'.");
      if (item.startsAt && item.endsAt && item.endsAt <= item.startsAt) throw movieError(400, "INVALID_RIGHTS_WINDOW", "Rights window end must be after its start.");
      return { territoryCode, rule: item.rule, startsAt: item.startsAt ?? null, endsAt: item.endsAt ?? null, note: item.note?.trim() || null };
    });
  }
  ...
  private normalizeLocalizations(items: MovieLocalizationInput[]) {
    ...
    return items.map((item) => {
      ...
      if (!/^[A-Za-z]{2,3}(?:-[A-Za-z0-9]{2,8})*$/.test(locale) || seen.has(locale.toLowerCase())) throw movieError(400, "INVALID_MOVIE_LOCALE", "Movie localization locale is invalid or duplicated.");
      seen.add(locale.toLowerCase());
      return { locale, title: item.title?.trim() || null, synopsis: item.synopsis?.trim() || null };
    });
  }

  private async assertReferencedMedia(primaryVideoId: string | null, trailerVideoId: string | null, artwork: MovieArtworkInput[]) {
    const videoIds = [...new Set([primaryVideoId, trailerVideoId].filter((value): value is string => Boolean(value)))];
    if (videoIds.length) {
      const count = await this.database.client.video.count({ where: { id: { in: videoIds } } });
      if (count !== videoIds.length) throw movieError(400, "VIDEO_REFERENCE_NOT_FOUND", "One or more referenced videos do not exist.");
    }
    const assetIds = [...new Set(artwork.map((item) => item.mediaAssetId))];
    if (assetIds.length) {
      const count = await this.database.client.mediaAsset.count({ where: { id: { in: assetIds }, removedAt: null } });
      if (count !== assetIds.length) throw movieError(400, "ARTWORK_ASSET_NOT_FOUND", "One or more artwork MediaAssets do not exist.");
    }
  }

  private async requireMovie(movieId: string) {
    const movie = await this.database.client.movie.findUnique({ where: { id: movieId }, select: { id: true } });
...
function toPublicMovie(movie: Awaited<ReturnType<MovieCatalogService["getAdminById"]>>) {
  return {
    ...
    primaryVideo: movie.primaryVideo ? { id: movie.primaryVideo.id, slug: movie.primaryVideo.slug, durationMs: movie.primaryVideo.durationMs } : null,
    trailerVideo: movie.trailerVideo?.status === "PUBLISHED" && movie.trailerVideo.visibility === "PUBLIC" ? { id: movie.trailerVideo.id, slug: movie.trailerVideo.slug, durationMs: movie.trailerVideo.durationMs } : null,
    localizations: movie.localizations.map((item) => ({ locale: item.locale, title: item.title, synopsis: item.synopsis })),
  };
}
...
function publicArtwork(item: Awaited<ReturnType<MovieCatalogService["getAdminById"]>>["artwork"][number] | null) {
  if (!item?.asset || item.asset.status !== "VALIDATED" || item.asset.removedAt) return null;
  return { mediaAssetId: item.mediaAssetId, objectKey: item.asset.r2ObjectKey, mimeType: item.asset.mimeType, width: item.asset.width, height: item.asset.height, altText: item.altText };
}

function requiredText(value: string, field: string, maxLength: number) {
  const result = value.trim();
  if (!result || result.length > maxLength) throw movieError(400, "INVALID_MOVIE_FIELD", `${field} is required and must be at most ${maxLength} characters.`);
  return result;
}

function checkedInteger(value: number, field: string, min: number, max: number) {
  if (!Number.isInteger(value) || value < min || value > max) throw movieError(400, "INVALID_MOVIE_FIELD", `${field} must be an integer between ${min} and ${max}.`);
  return value;
}
...
function movieError(status: number, code: string, message: string, extra?: Record<string, unknown>) {
