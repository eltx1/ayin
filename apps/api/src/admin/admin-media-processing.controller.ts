...
const recoverySchema = z
  .object({ mode: z.enum(ADAPTIVE_RECOVERY_MODES), batchSize: z.number().int().min(1).max(20).optional() })
  .strict();
...
export class AdminMediaProcessingController {
  constructor(
    ...
    @Inject(MediaAdaptiveRolloutService) private readonly adaptiveRollout: MediaAdaptiveRolloutService,
  ...
  async overview() {
    const [overview, jobs] = await Promise.all([
      ...
      this.database.client.mediaProcessingJob.findMany({
        ...
        take: 100,
        select: {
          id: true, videoId: true, generation: true, status: true, stage: true,
          progressPercent: true, attempt: true, priority: true, leaseOwner: true,
          leaseExpiresAt: true, errorCode: true, errorMessage: true, queuedAt: true,
  ...
  async runAdaptiveBackfill(@Req() request: AdminAuthenticatedRequest, @Body() body: unknown) {
    ...
    if (!parsed.success) throw adminBadRequest("INVALID_BACKFILL_BATCH", "Backfill batch request is invalid.");
  ...
  async recoverAdaptive(@Req() request: AdminAuthenticatedRequest, @Body() body: unknown) {
    ...
    if (!parsed.success) throw adminBadRequest("INVALID_ADAPTIVE_RECOVERY", "Adaptive recovery request is invalid.");
  ...
  async retryFailed(@Req() request: AdminAuthenticatedRequest, @Param("jobId") jobIdRaw: string) {
    ...
    return this.database.client.$transaction(async (tx) => {
      ...
      const retried = await tx.mediaProcessingJob.update({
        where: { id: job.id },
        data: {
          status: "QUEUED", stage: "ADMIN_RETRY_QUEUED", progressPercent: 0, attempt: 0,
          queuedAt: new Date(), startedAt: null, completedAt: null, leaseOwner: null,
          leaseExpiresAt: null, heartbeatAt: null, errorCode: null, errorMessage: null,
        },
      });
  ...
  async reprocess(@Req() request: AdminAuthenticatedRequest, @Param("videoId") videoIdRaw: string) {
    ...
    return this.database.client.$transaction(async (tx) => {
      const latest = await tx.mediaProcessingJob.findFirst({
        where: { videoId }, orderBy: { generation: "desc" }, select: { status: true },
      });
      if (latest && !terminalStatuses.has(latest.status)) {
        throw adminBadRequest("MEDIA_PROCESSING_ALREADY_ACTIVE", "This video already has an active or queued processing generation.");
      }
      const job = await this.lifecycle.createReprocessJob(tx, videoId);
      if (!job) {
        throw adminBadRequest("REPROCESS_SOURCE_UNAVAILABLE", "A validated playback source is required before this video can be reprocessed.");
      }
      await this.audit.recordInTransaction(tx, {
        actorAccountId: request.ayinAuth.accountId,
