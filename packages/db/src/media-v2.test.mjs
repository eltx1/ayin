...
const schema = readFileSync(new URL("../prisma/media-v2.prisma", import.meta.url), "utf8");
...
describe("Task 39 media architecture V2 persistence", () => {
  it("models durable generation and rendition lifecycle state", () => {
    ...
    expect(schema).toContain("processingVersion    Int                           @default(2)");
  ...
  it("enforces deterministic generation/rendition identities and object-key uniqueness", () => {
    ...
    expect(schema).toContain("fallbackR2ObjectKey  String                        @unique");
    expect(schema).toContain("hlsMasterR2ObjectKey String                        @unique");
    expect(schema).toContain("playlistR2ObjectKey  String                    @unique");
    expect(schema).toContain("segmentR2Prefix      String                    @unique");
  });
  ...
  it("keeps the migration additive and leaves live V1 media tables untouched", () => {
    ...
    expect(migration).toContain('REFERENCES "MediaPlaybackGeneration"("id") ON DELETE CASCADE');
